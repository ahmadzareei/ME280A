function time_dependent_lumped
...
for element =1:size(e,1)
     nodes = p(e(element,:),:);
     if( abs(nodes(2,1).^2 + nodes(2,2).^2 - ro.^2) < 0.00001)
        boundary(ll,:) = [e(element,2),e(element,3)];ll=ll+1;
     end
end
...
%    if(mod(floor(t/dt),1) == 0)
        TSol(end+1) = Sol(boundary(1,1));  
%    end
    
%     if(mod(floor(t/dt),500) == 0)
%         clf;
%         patch('Faces',e,'Vertices',[p,Sol],'FaceVertexCData',Sol,'FaceCol','interp');colorbar; axis square
%         set(gca,'FontSize',16);
%         xlabel('x','FontSize',16);
...
% plot([1:100:size(TSol,2)]*0.01, TSol(1:100:end),'r','LineWidth',2); hold on;
% plot([1:100:size(TSol,2)]*0.01,-231*ones(size([1:100:size(TSol,2)])),'b--','LineWidth',2);
% legend('time dependent', 'Steady state solution')
% set(gca,'FontSize',16);
% xlabel('time','FontSize',16);
% ylabel('y','FontSize',16);
% xlim([0,100]);
% title('T(ro) vs time','Fontsize',16);
% print -depsc Tovstime.eps
% 
% plot([1:1:size(TSol,2)/20]*0.01, TSol(1:1:end/20),'r','LineWidth',2); hold on;
% plot([1:1:size(TSol,2)/20]*0.01,-230*ones(size([1:1:size(TSol,2)/20])),'b--','LineWidth',2);
% legend('time dependent', 'Steady state solution')
% set(gca,'FontSize',16);
% xlabel('time','FontSize',16);
% ylabel('y','FontSize',16);
